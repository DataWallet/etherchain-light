...
router.get('/pending', function(req, res, next) {
  ...
  async.waterfall([
    ...
    res.render('tx_pending', { txs: txs, netstatsUrl: config.netstatsUrl  });
  });
});
...
router.get('/submit', function(req, res, next) {
  var config = req.app.get('config');
  res.render('tx_submit', { netstatsUrl: config.netstatsUrl });
});

router.post('/submit', function(req, res, next) {
  if (!req.body.txHex) {
    return res.render('tx_submit', { message: "No transaction data specified", netstatsUrl: config.netstatsUrl });
  }
  ...
  async.waterfall([
    ...
    if (err) {
      res.render('tx_submit', { message: "Error submitting transaction: " + err, netstatsUrl: config.netstatsUrl  });
    } else {
      res.render('tx_submit', { message: "Transaction submitted. Hash: " + hash, netstatsUrl: config.netstatsUrl  });
    }
  });
});
...
router.get('/:tx', function(req, res, next) {
  ...
  async.waterfall([
    ...
    res.render('tx', { tx: tx, netstatsUrl: config.netstatsUrl  });
  });
...
router.get('/raw/:tx', function(req, res, next) {
  ...
  async.waterfall([
    ...
    res.render('tx_raw', { tx: tx, netstatsUrl: config.netstatsUrl  });
  });
});
